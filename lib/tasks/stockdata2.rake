...
namespace :db do
  ...
  def getPriceKabutan(ticker)
    ...
    #name = doc.css(".kobetsu_data_table1_meigara").text
		name = doc.css("#kobetsu").text.gsub(/\(.*/, "")#tagがclassになっていたので#に変更して「(」以降を削除した文字列を名前とする

		p "ticker = #{ticker.to_s}, name=#{name.to_s}"
		# 最新データの取得
		#price0 = doc.css("#stock_kabuka0 > tr")
		price0 = doc.css(".stock_kabuka0 > tbody > tr")
  	...
  	#price_series = doc.css(".stock_kabuka1 > tr")
		...
		#p "price_series = #{price_series}"
  	price_series.each do |record|
			th_tags = record.css('th')#日付
  		td_tags = record.css('td')#OHLCV配列
			# p "th_tags = #{th_tags}"
			# p "td_tags = #{td_tags}"
