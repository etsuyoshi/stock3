  ...
  def get_schedules
    ...
    page.css('table.ei-list').each do |list_day| #日付ごと
      ...
      list_day.css('tr.ei-list-item').each do |list_item|
        ...
        # 予想がない場合がある
        description = "#{Time.at(list_time).strftime('%-m月%-d日')}に#{title}が発表されます。"
        if expects.nil? || expects.to_s == ""
          description = "#{description}#{(before.to_s=="") ? "" : "前回は#{before}と発表された際、為替は#{affects}動きました。"}"
        else
          description = "#{description}今回の予想は#{expects}で #{(before.to_s=="") ? "す。" : "前回は#{before}と発表された際、為替は#{affects}動きました。"}"
        end
